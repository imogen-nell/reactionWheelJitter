...
load('parameters.mat')
%%%%%%%%%%%%%%%%%%%%%%
%%only w2 and wA3 chage  of W_Frame
...
%hold off;
figure;
plot(tspan, store_Wframe2(1, :), '-b', 'DisplayName', 'Component 2');
hold on;
title('(W) Motor frame components', 'FontSize', 14, 'FontWeight', 'bold', 'Color', 'g');
plot(tspan, store_Wframe2(2, :), '-b', 'DisplayName', 'Component 2');
plot(tspan, store_Wframe2(3, :), '-b', 'DisplayName', 'Component 2');
plot(tspan, store_Wframe3(1, :), '-g', 'DisplayName', 'Component 2');
plot(tspan, store_omega, '-r', 'DisplayName', 'Component 2');
plot(tspan, store_Wframe3(2, :), '-g', 'DisplayName', 'Component 2');
plot(tspan, store_Wframe3(3, :), '-g', 'DisplayName', 'Component 2');
hold off;
%%%%%%%%%%%%%%%%%%%%%%
...
function dx = func(t,y)
    ...
    %dy = x = [ w_b_n_d , rdd     , Wd , wframe2d , wframe3 d  ]  
    ...
    irw_b_dash  = irw_wc_dash + mrw * skew(r_wc_b_dash)*skew( r_wc_b)' + mrw * skew(r_wc_b)*skew( r_wc_b_dash)'; %(35)
    ...
    tau_rhs = - skew(w_b_n) * isc_b*w_b_n - isc_b_dash * w_b_n - msc * skew(c) * ...
        ...
        - (irw_wc*W_frame(:,1) + mrw * d * (skew(r_wc_b) - skew(c)) * W_frame(:,3))* e * v +Lb; %(81)
    ...
    %r_wc_b_ddash = cross(W*W_frame(:,1), d * W * W_frame(:,3));
    r_wc_b_ddash = d *Wd *W_frame(:,3) - d * W^2 * W_frame(:,2); %(8)
    cddash       = (mrw/msc) * r_wc_b_ddash;  %(5)
    

    
    cdd          = cddash + 2 * cross(w_b_n, c) + cross(x1,c) +...
        cross(w_b_n,cross(w_b_n,c)); %(9)

    x2 = r_c_ndd - cdd;             %(2)  -
    x3 = [Wd 0 0]';                 %(74) 
    x4 =  W * W_frame(:,3) ;        %(27) 56?? body-frame derivatives 
    x5 = -W * W_frame(:,2) ;        %(28) -
    
    %r_wc_bdd = d * x3(1) * W_frame(:,3) - d * W^2 * W_frame(:,2) + cross(x1, r_wc_b)...
    %    + 2 * cross(w_b_n, d*W*W_frame(:,3)) + cross(w_b_n,cross(w_b_n,r_wc_b)); %(33)
    %r_bc_bdd = cross(x1, r_bc_b) + cross(w_b_n,cross(w_b_n, r_bc_b)); %(20)
    %Hhub_bd  = ihub_bc * x1 + cross(w_b_n, ihub_bc * w_b_n) +...
    %    mhub * cross(r_bc_b,r_bc_bdd); %(18)
    %Hrw_bd   = irw_wc_dash * (w_b_n + W * W_frame(:,1)) + irw_wc(x1+x3(1) * W_frame(:,1)) ...
    %    + cross(w_b_n, irw_wc * (w_b_n + W*W_frame(:,1)))+mrw*cross(r_wc_b, r_wc_bdd); %(23)
    %Hsc_bd   = Hhub_bd + Hrw_bd;
