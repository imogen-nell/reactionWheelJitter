%mod 9 - cleaning up 8, remove wframe from y 
% also add dynamic us 
...
store_forcedw2(:,1) = w2_0;
store_forcedw3(:,1) = w3_0;
...
for i=1:tspan(end)/dt
    ...
    theta                = yout(2,3);
    %mod(yout(2,3),360);
    store_omega(2,i+1)   = mod(theta,360);
...
plot(tspan, store_W(2, :), '-g', 'DisplayName', 'Component 1');
plot(tspan, store_W(3, :), '-g', 'DisplayName', 'Component 1');
hold off;
...
plot(tspan, store_omega(1,:), '-r', 'DisplayName', 'RW Omega');%%omega
plot(tspan, store_omega(2,:), '-b', 'DisplayName', 'Theta');%%theta
hold off;
