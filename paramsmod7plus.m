...
r_bc_b = [0 0 0]'; %hub COM location wrt B, point on sc

%wheel properties
g   = [1 0 0]' ; %[.7887 -.2113 -1/sqrt(3)]';
W_frame_init = [g, cross(g,[0 0 1]')/norm(cross(g,[0 0 1]')), cross(cross(g,[0 0 1]'),g )/norm(cross(cross(g,[0 0 1]'),g ))]; %wheel orientation matrix

U_s = 0.0000048 ; %wheel static imbal
U_d = 0.0000154 ; %dynamic imbal
d   =  U_s/mrw; % gc of rw to com  (not constant) (cm estimate)
W_init = 10000* 2*pi/60; % cap omega (random rad/s chosen) (initial wheel speed
us_0 = 0; 
...
r_w_b = [0 0 0]' ; %[0.6309 -0.1691 0.4619]';%location vector of rw. B frame origin to rw origin / gc 
J11 = irw_wc(1,1);
J12 = 0;
J13 = U_d;
J22 = irw_wc(2,2);
